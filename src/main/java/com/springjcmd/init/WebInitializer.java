...
public class WebInitializer implements WebApplicationInitializer {
	...
	public void onStartup(ServletContext servletContext) throws ServletException {
		...
		ServletRegistration.Dynamic mvcServletRegistration = servletContext.addServlet("mvcServletRegistration",
				mvcDispatcherServlet);
		// 서블릿 로딩 중 우선순위 보장
		mvcServletRegistration.setLoadOnStartup(1);
		// 디스패처 서블릿은 .do로 끝나는 모든 요청을을 처리하도록 매핑
		mvcServletRegistration.addMapping("/springJCMD/*");
		...
		// 인코딩 타입 설정 (CharacterEncodingFilter의 매개변수 forceEncoding 값 초기화, forceEncoding :
		// 모든 요청과 응답에서도 필터 인코딩 적용 여부)
		characterEncodingFilter.setInitParameter("forceEncoding", "true");

		// HTML TAG 보안 필터
		FilterRegistration.Dynamic htmlTagFilter = servletContext.addFilter("htmlTagFilter", new HTMLTagFilter());
		// HTML TAG 보안 필터 적용 범위 설정 (필터가 적용될 디스패처 타입, 필터가 서블릿 매핑 앞에 적용될지 여부, 필터가 적용될 URL
		// 패턴)
	...
	private void _setActiveProfile(AnnotationConfigWebApplicationContext rootContext) {
		...
		if (activeProfile.length == 0) {
			...
			LOGGER.info("{}", EnumProfile.LOCAL_SERVER.getValue());
